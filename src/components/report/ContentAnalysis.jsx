...
export default function ContentAnalysis({ reportId = 'b1' }) {
        ...
        <Card>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={content.postTypeBreakdown}
                  cx="50%" cy="50%"
                  outerRadius={80}
                  paddingAngle={4}
                  dataKey="value"
                  ...
                  labelLine={false}
                >
                  {content.postTypeBreakdown.map((_, i) => (
          ...
          <div className="flex flex-wrap gap-2 items-center h-48 content-center">
            {content.topThemes.map((theme, i) => {
              ...
              return (
                <span
                  key={theme}
                  className={`${size} px-3 py-1 rounded-full bg-[--bg-tertiary] border border-[--border] text-[--text-secondary] hover:text-[--accent] hover:border-[--accent]/30 transition-colors cursor-default font-body`}
                >
                  {theme}
                </span>
              )
        ...
        <div className="overflow-x-auto">
            ...
            {Array.from({ length: 12 }).map((_, w) => (
              <div key={w} className="flex-1 space-y-[3px]">
                {Array.from({ length: 7 }).map((_, d) => {
                  const cell = content.postingFrequency.find((c) => c.week === w && c.day === d)
                  ...
                  return (
                    <div
                      ...
                      style={{
                        backgroundColor: count === 0 ? 'var(--bg-tertiary)' :
                          count <= 1 ? 'rgba(0,212,170,0.2)' :
                          count <= 2 ? 'rgba(0,212,170,0.4)' :
                          count <= 3 ? 'rgba(0,212,170,0.6)' :
                          'rgba(0,212,170,0.9)'
                      }}
          ...
          <div className="mt-2 flex items-center gap-2 text-[10px] text-[--text-muted] font-body">
            ...
            <div className="flex gap-[2px]">
              {[0, 0.2, 0.4, 0.6, 0.9].map((o) => (
                <div key={o} className="w-3 h-3 rounded-sm" style={{ backgroundColor: o === 0 ? 'var(--bg-tertiary)' : `rgba(0,212,170,${o})` }} />
              ))}
            </div>
            <span>More</span>
          </div>
      ...
      <Card>
        ...
        <div className="flex items-start gap-4">
          <div className="w-12 h-12 rounded-lg bg-[--accent-glow] flex items-center justify-center shrink-0">
            <span className="text-xs font-bold text-[--accent] font-display">VIDEO</span>
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-4 mt-2 text-xs text-[--text-muted] font-body">
              <span>{content.bestPost.reach.toLocaleString()} reach</span>
              <span>{content.bestPost.engagement.toLocaleString()} engagements</span>
