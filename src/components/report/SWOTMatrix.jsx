  ...
  return (
    <section>
      <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} className="grid sm:grid-cols-2 gap-4">
        {items.map((list, i) => (
          <motion.div key={quadrantStyles[i].label} variants={quadrantVariants} className={`border ${quadrantStyles[i].border} ${quadrantStyles[i].bg} rounded-xl p-5`}>
            <h4 className={`text-sm font-semibold font-display mb-3 ${quadrantStyles[i].labelColor}`}>{quadrantStyles[i].label}</h4>
