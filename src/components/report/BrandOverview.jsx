...
export default function BrandOverview({ reportId = 'b1' }) {
  const data = mockReportData(reportId)
  const { brand } = data

  const stats = [
    { icon: Users, label: 'Followers', value: formatNumber(brand.followers) },
    { icon: Eye, label: 'Avg Weekly Reach', value: formatNumber(brand.avgWeeklyReach) },
    { icon: BarChart3, label: 'Engagement Rate', value: `${brand.engagementRate}%` },
  ]
  ...
  return (
    <section>
      ...
      <Card>
        ...
        <p className="text-sm text-[--text-secondary] font-body leading-relaxed">{brand.description}</p>
      </Card>
    </section>
  )
}
